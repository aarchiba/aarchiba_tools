...
def test_downsample_1d():
    assert_almost_equal(downsample([1,2,3,4],2),
                        [1.5,3.5])
    assert_almost_equal(downsample([1,2,3,4],4),
                        [2.5])
    assert_almost_equal(downsample([1,2,3,4],1),
                        [1,2,3,4])
...
def test_downsample_negative_axes():
    ...
    assert_almost_equal(downsample(a,11,axis=2),
                        downsample(a,11,axis=-1))
    assert_almost_equal(downsample(a,3,axis=0),
                        downsample(a,3,axis=-3))
